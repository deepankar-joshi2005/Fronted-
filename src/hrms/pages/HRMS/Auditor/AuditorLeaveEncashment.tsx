...
import { Wallet, CheckCircle2, Clock, IndianRupee, Search } from "lucide-react";
...
const statusStyles: Record<Encashment["status"], string> = {
...
export default function AuditorLeaveEncashment() {
  const [list, setList] = useState<Encashment[]>([]);
  ...
  useEffect(() => {
    const fetchList = async () => {
      try {
        setLoading(true);
        const res = await axios.get(`${API}/leave-encashment`, { headers });
        setList(res.data || []);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  const summary = useMemo(
    () => ({
      ...
      totalAmount: list.reduce((s, r) => s + (r.totalAmount || 0), 0),
    }),
    [list]
  );
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-[var(--muted-foreground)]">Encashment payouts against unused leave balances</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        <StatCard label="Total Requests" value={summary.total} icon={Wallet} tone="primary" />
        ...
        <StatCard label="Total Payout" value={currency(summary.totalAmount)} icon={IndianRupee} tone="violet" />
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="card-premium shadow-premium-sm flex flex-col items-center justify-center gap-2 p-16 text-center">
          ...
          <p className="text-sm text-[var(--muted-foreground)]">No encashment requests match your search</p>
        </div>
      ) : (
                ...
                {paginated.map((r) => (
                  <tr key={r._id} className="transition-colors hover:bg-[var(--muted)]">
                    <td className="px-4 py-3.5">
                      ...
                      <p className="text-xs text-[var(--muted-foreground)]">{r.employee?.employeeId}</p>
                    </td>
                    <td className="px-4 py-3.5 text-[var(--foreground)]">{r.leaveType}</td>
                    <td className="px-4 py-3.5 text-[var(--foreground)]">{r.requestedDays}</td>
                    <td className="px-4 py-3.5 font-semibold text-[var(--foreground)]">{currency(r.totalAmount)}</td>
                    <td className="px-4 py-3.5">
                    ...
                    <td className="px-4 py-3.5 text-[var(--muted-foreground)]">
                      {r.status === "APPROVED" ? (r.paymentStatus === "PAID" ? "Paid" : "Unpaid") : "—"}
                    </td>
                  </tr>
                ))}
              </tbody>
