...
const ViewSalaryStructureModal = ({ isOpen, onClose, data }: Props) => {
  ...
  // Always show Basic/HRA/Other Earnings + Employee PF/Professional Tax/TDS
  // even when zero (the usual baseline components); every other component
  // only shows once it actually has a value, so the panel doesn't list 20+
  // zero rows for a simple structure.
  const alwaysShown = new Set(["basic", "hra", "otherAllowance", "pf", "professionalTax", "tds"]);
  const earningRows = EARNING_COMPONENTS.filter(([key]) => alwaysShown.has(key) || data[key]);
  const deductionRows = DEDUCTION_COMPONENTS.filter(([key]) => alwaysShown.has(key) || data[key]);

  const totalEarnings = calcTotalEarnings(data);
  const totalDeductions = calcTotalDeductions(data);
  const netSalary = calcNetSalary(data);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div>
              ...
              <div className="space-y-3">
                {earningRows.map(([key, label]) => (
                  <div key={key} className="flex justify-between items-center text-sm">
                    <span className="text-gray-500">{label}</span>
                    <span className="font-semibold text-gray-800">₹{(data[key] || 0).toLocaleString()}</span>
                  </div>
                ))}
