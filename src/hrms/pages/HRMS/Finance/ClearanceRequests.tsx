...
const FinanceClearanceRequests = () => {
  ...
  /* 🔹 FETCH CLEARANCE REQUESTS (auto-created the moment a resignation is approved) */
  const fetchClearance = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get(`/clearances`, {
        params: { page, search },
      });
      ...
      toast({ type: "error", title: "Fetch Failed", message: "Failed to load clearance requests" });
    } finally {
      setLoading(false);
    }
  };
  ...
  /* 🔹 OPEN REQUEST DIALOG */
  const openRequest = (row: ClearanceRow) => {
    setSelected(row);
    const finance = row.clearances.find((c) => c.department === "Finance");
    setFinanceStatus(finance?.status || "PENDING");
    setFinanceRemarks(finance?.remarks || "");
  };

  /* 🔹 FINANCE UPDATES ITS OWN CLEARANCE STATUS */
  const handleUpdateFinance = async () => {
    if (!selected) return;
    try {
      setFinanceSaving(true);
      await axiosInstance.patch(`/clearances/${selected._id}/department`, {
        departmentName: "Finance",
        status: financeStatus,
        remarks: financeRemarks,
      });
      toast({ type: "success", title: "Finance Clearance Updated", message: "Finance status has been synchronized." });
      setSelected((prev) =>
        prev
          ? {
              ...prev,
              clearances: prev.clearances.map((c) =>
                c.department === "Finance" ? { ...c, status: financeStatus, remarks: financeRemarks } : c
              ),
            }
          : prev
      );
      ...
      toast({ type: "error", title: "Update Failed", message: "Could not update Finance clearance status" });
    } finally {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-[var(--muted-foreground)]">
            Exiting employees awaiting Finance department sign-off
          </p>
        </div>
      </div>
      ...
      <div className="card-premium shadow-premium-sm overflow-hidden relative min-h-[300px]">
        ...
        {data.length === 0 && !loading ? (
          <div className="flex flex-col items-center justify-center gap-2 p-16 text-center">
            ...
            <p className="text-sm text-[var(--muted-foreground)]">No clearance requests align with current filters.</p>
          </div>
        ) : (
            ...
            <table className="w-full min-w-[720px] text-sm">
              <thead className="bg-[var(--muted)]">
                <tr className="text-left text-xs font-semibold uppercase tracking-wide text-[var(--muted-foreground)]">
                  ...
                  <th className="px-4 py-3">Finance Status</th>
                  <th className="px-4 py-3 text-center">Overall Clearance</th>
              ...
              <tbody className="divide-y divide-[var(--border)]">
                {data.map((row) => {
                  const financeDept = row.clearances.find((c) => c.department === "Finance");
                  const financeDeptStatus = financeDept?.status || "PENDING";
                  ...
                  return (
                    <tr key={row._id} className="transition-colors hover:bg-[var(--muted)]">
                      ...
                      <td className="px-4 py-3.5">
                        <span className={cn("inline-flex rounded-md px-2 py-0.5 text-xs font-medium", deptPillClass[financeDeptStatus])}>
                          {financeDeptStatus}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3.5 text-right">
                        <button
                          ...
                          className="inline-flex h-8 items-center gap-1 rounded-lg border border-[var(--border)] px-3 text-xs font-medium text-[var(--foreground)] transition-colors hover:border-[var(--primary)] hover:text-[var(--primary)]"
                        >
                          Review
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-[var(--border)] bg-[var(--card)] p-0 shadow-premium-lg">
          <DialogHeader className="border-b border-[var(--border)] bg-[var(--muted)] p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <DialogTitle className="text-lg font-semibold text-[var(--foreground)]">Clearance Request</DialogTitle>
          ...
          {selected && (
            <div className="p-6 space-y-6">
              {/* STAFF OVERVIEW */}
              <div className="grid grid-cols-2 md:grid-cols-3 gap-5 rounded-xl border border-[var(--border)] bg-[var(--muted)] p-5">
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          isFinance ? "border-[var(--primary)]" : "border-[var(--border)]"
                        )}
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span className={cn("flex h-7 w-7 items-center justify-center rounded-lg", deptPillClass[c.status])}>
                          ...
                          {isFinance ? (
                            <select
                              ...
                              onChange={(e) => setFinanceStatus(e.target.value as Status)}
                        ...
                        {isFinance ? (
                          <div className="space-y-2 border-t border-dashed border-[var(--border)] pt-3">
                            <input
                              ...
                              placeholder="Finance remarks (optional)"
                            ...
                            >
                              {financeSaving ? "Saving..." : "Save Finance Status"}
                            </button>
                          </div>
                        ) : (
                          c.remarks && (
...
export default FinanceClearanceRequests;
